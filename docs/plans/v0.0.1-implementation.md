...
**Architecture:** Pure-function scheduling checks in `internal/diagnosis/`, nodepool grouping in `internal/nodepool/`, GOATScaler event reading in `internal/autoscaler/`, PodReconciler in `internal/controller/` wiring everything together. All scheduling checks are tested as pure functions (no K8s client). Controller integration tested with envtest.
...
Use controller-runtime fake client (not envtest yet; that is Task 13).
...
const testNodepoolLabel = "node.kubernetes.io/nodepool"
...
func TestReconcile_PendingPod(t *testing.T) {
	...
	node := &corev1.Node{
		ObjectMeta: metav1.ObjectMeta{
			Name:   "node-1",
			Labels: map[string]string{testNodepoolLabel: "pool-a", "name": "general"},
		},
		Status: corev1.NodeStatus{
			Allocatable: corev1.ResourceList{
				corev1.ResourceCPU: resource.MustParse("4"),
...
func (r *PodReconciler) diagnose(ctx context.Context, pod *corev1.Pod) ([]diagnosis.NodepoolDiagnosis, error) {
	...
	pools, err := collector.Collect(ctx, r.Client, r.NodepoolLabel, r.NameLabel)
	if err != nil {
...
## Task 12: Wire reconciler into main.go

**File:** `cmd/main.go`

**What to change:**

1. Import `internal/autoscaler` and `internal/controller`
2. After `createManager()`, call `autoscaler.Detect(ctx, mgr.GetClient())` to detect autoscaler
3. Create `PodReconciler` with fields from `managerOptions`
4. Call `controller.SetupWithManager(mgr, reconciler)`
5. Add `nameLabel` field to `managerOptions` with flag `--name-label` defaulting to `"name"`
...
func main() {
	...
	reader, err := autoscaler.Detect(ctx, mgr.GetClient())
	if err != nil {
...
Also add to `parseFlags`:

```go
flag.StringVar(&opts.nameLabel, "name-label", "name", "label key for nodepool human-readable name")
```

And add `nameLabel string` to the `managerOptions` struct.
...
func main() {
	...
	if err := setupReconciler(mgr, opts, reader); err != nil {
		slog.Error("unable to setup controller", "error", err)
		os.Exit(1)
	}
...
func setupReconciler(mgr ctrl.Manager, opts managerOptions, reader autoscaler.StatusReader) error {
	reconciler := &controller.PodReconciler{
		Client:          mgr.GetClient(),
		Recorder:        mgr.GetEventRecorderFor("fitcheck"),
		NodepoolLabel:   opts.nodepoolLabel,
...
## Task 13: envtest integration test
...
func TestReconciler_Envtest(t *testing.T) {
	...
	gpuNode := &corev1.Node{
		ObjectMeta: metav1.ObjectMeta{
			Name: "gpu-node-1",
			Labels: map[string]string{
				"node.kubernetes.io/nodepool": "gpu-pool",
				"name":                        "GPU Pool",
			},
		},
	...
	cpuNode := &corev1.Node{
		ObjectMeta: metav1.ObjectMeta{
			Name: "cpu-node-1",
			Labels: map[string]string{
				"node.kubernetes.io/nodepool": "cpu-pool",
				"name":                        "CPU Pool",
			},
		},
		Status: corev1.NodeStatus{
...
No new implementation needed. The reconciler from Task 11 should handle this. If adjustments are needed, update the reconciler.
