...
**Architecture:** All packages under `internal/` following Go official layout. `cmd/main.go` is a thin entry point: parse flags, resolve provider, call `app.Run()`. Scheduling checks in `internal/diagnosis/` (pure functions), provider abstraction in `internal/provider/`, nodepool grouping in `internal/nodepool/`, autoscaler readers in `internal/autoscaler/`, app lifecycle in `internal/app/`, PodReconciler in `internal/controller/`. Adding a new cloud provider means implementing `internal/provider.Provider` and registering it.
...
**Started:** 2026-07-13 21:14 WIB
**Last Updated:** 2026-07-13 21:14 WIB
...
## Task 0: Foundation refactor (extract app lifecycle from main.go)
...
Extract manager creation, health checks, and cache options from cmd/main.go. The app layer only knows about business config. Infra concerns (bind address, kubeconfig) are passed in by the caller.
...
// Options holds business configuration for the controller.
// Infra concerns (bind address, kubeconfig) are not here.
type Options struct {
...
// cfg is the Kubernetes client config. bindAddr is the single HTTP port
// serving /metrics, /healthz, and /readyz.
func Run(cfg *rest.Config, bindAddr string, opts Options) error {
	mgr, err := createManager(cfg, bindAddr, opts)
	if err != nil {
...
func createManager(cfg *rest.Config, bindAddr string, opts Options) (ctrl.Manager, error) {
	mgr, err := ctrl.NewManager(cfg, ctrl.Options{
		Metrics: metricsserver.Options{
			BindAddress:   bindAddr,
			ExtraHandlers: map[string]http.Handler{},
		},
		HealthProbeBindAddress: bindAddr,
...
Note: controller-runtime serves health probes and metrics on the same bind address when `HealthProbeBindAddress` matches `Metrics.BindAddress`. The paths are `/healthz`, `/readyz`, and `/metrics` on a single port.
...
func main() {
	...
	if err := app.Run(cfg, bindAddr, opts); err != nil {
		slog.Error("fatal", "error", err)
		os.Exit(1)
	}
}

func parseFlags() (string, app.Options) {
	var (
		bindAddr string
		opts     app.Options
	)

	flag.StringVar(&bindAddr, "bind-addr", ":8080", "address for metrics, healthz, readyz")
...
```bash
git add cmd/ internal/log/ internal/app/
...
**Test file:** `internal/diagnosis/types_test.go`
...
```go
// internal/diagnosis/types_test.go
...
Run: `go test ./internal/diagnosis/... -v -run TestNodepoolDiagnosis`
...
Run: `go test ./internal/diagnosis/... -v`
Expected: ALL PASS

- [ ] **Step 5: Lint**

Run: `go tool golangci-lint run ./internal/diagnosis/...`
Expected: zero issues

- [ ] **Step 6: Commit**

```bash
git add internal/diagnosis/types.go internal/diagnosis/types_test.go
...
**File:** `internal/diagnosis/taint.go`
...
**Test file:** `internal/diagnosis/taint_test.go`
...
```go
// internal/diagnosis/taint_test.go
package diagnosis_test
...
Run: `go test ./internal/diagnosis/... -v -run TestCheckTaints`
Expected: FAIL (function does not exist)

- [ ] **Step 3: Write minimal implementation (GREEN)**

```go
// internal/diagnosis/taint.go
package diagnosis
...
```bash
git add internal/diagnosis/taint.go internal/diagnosis/taint_test.go
...
**File:** `internal/diagnosis/selector.go`
...
**Test file:** `internal/diagnosis/selector_test.go`
...
```go
// internal/diagnosis/selector_test.go
package diagnosis_test
...
Run: `go test ./internal/diagnosis/... -v -run TestCheckNodeSelector`
Expected: FAIL (function does not exist)

- [ ] **Step 3: Write minimal implementation (GREEN)**

```go
// internal/diagnosis/selector.go
package diagnosis
...
```bash
git add internal/diagnosis/selector.go internal/diagnosis/selector_test.go
...
**File:** `internal/diagnosis/affinity.go`
...
**Test file:** `internal/diagnosis/affinity_test.go`
...
```go
// internal/diagnosis/affinity_test.go
package diagnosis_test
...
Run: `go test ./internal/diagnosis/... -v -run TestCheckNodeAffinity`
Expected: FAIL (function does not exist)

- [ ] **Step 3: Write minimal implementation (GREEN)**

```go
// internal/diagnosis/affinity.go
package diagnosis
...
```bash
git add internal/diagnosis/affinity.go internal/diagnosis/affinity_test.go
...
**File:** `internal/diagnosis/resource.go`
...
**Test file:** `internal/diagnosis/resource_test.go`
...
```go
// internal/diagnosis/resource_test.go
package diagnosis_test
...
Run: `go test ./internal/diagnosis/... -v -run TestCheckResources`
Expected: FAIL (function does not exist)

- [ ] **Step 3: Write minimal implementation (GREEN)**

```go
// internal/diagnosis/resource.go
package diagnosis
...
```bash
git add internal/diagnosis/resource.go internal/diagnosis/resource_test.go
...
**File:** `internal/diagnosis/diagnose.go`
...
**Test file:** `internal/diagnosis/diagnose_test.go`
...
```go
// internal/diagnosis/diagnose_test.go
package diagnosis_test
...
Run: `go test ./internal/diagnosis/... -v -run TestDiagnoseNodepool`
Expected: FAIL (function does not exist)

- [ ] **Step 3: Write minimal implementation (GREEN)**

```go
// internal/diagnosis/diagnose.go
package diagnosis
...
```bash
git add internal/diagnosis/diagnose.go internal/diagnosis/diagnose_test.go
...
**File:** `internal/diagnosis/diagnose_all.go`
...
**Test file:** `internal/diagnosis/diagnose_all_test.go`
...
```go
// internal/diagnosis/diagnose_all_test.go
package diagnosis_test
...
Run: `go test ./internal/diagnosis/... -v -run TestDiagnoseAll`
Expected: FAIL (function does not exist)

- [ ] **Step 3: Write minimal implementation (GREEN)**

```go
// internal/diagnosis/diagnose_all.go
package diagnosis
...
```bash
git add internal/diagnosis/diagnose_all.go internal/diagnosis/diagnose_all_test.go
...
**File:** `internal/nodepool/collector.go`
...
**Test file:** `internal/nodepool/collector_test.go`
...
```go
// internal/nodepool/collector_test.go
...
Run: `go test ./internal/nodepool/... -v -run TestCollector`
Expected: FAIL (package does not exist)

- [ ] **Step 3: Write minimal implementation (GREEN)**

```go
// internal/nodepool/collector.go
package nodepool
...
Run: `go test ./internal/nodepool/... -v`
Expected: ALL PASS

- [ ] **Step 5: Lint**

Run: `go tool golangci-lint run ./internal/nodepool/...`
Expected: zero issues

- [ ] **Step 6: Commit**

```bash
git add internal/nodepool/collector.go internal/nodepool/collector_test.go
...
**File:** `internal/autoscaler/goatscaler.go`
...
**Test file:** `internal/autoscaler/goatscaler_test.go`
...
```go
// internal/autoscaler/goatscaler_test.go
...
Run: `go test ./internal/autoscaler/... -v -run TestGOATScaler`
Expected: FAIL (package does not exist)
...
```go
// internal/autoscaler/goatscaler.go
package autoscaler
...
Run: `go test ./internal/autoscaler/... -v`
Expected: ALL PASS

- [ ] **Step 5: Lint**

Run: `go tool golangci-lint run ./internal/autoscaler/...`
Expected: zero issues

- [ ] **Step 6: Commit**

```bash
git add internal/autoscaler/autoscaler.go internal/autoscaler/goatscaler.go internal/autoscaler/goatscaler_test.go
...
**File:** `internal/autoscaler/autoscaler.go` (extend existing file)
...
**Test file:** `internal/autoscaler/detect_test.go`
...
```go
// internal/autoscaler/detect_test.go
package autoscaler_test
...
Run: `go test ./internal/autoscaler/... -v -run TestDetect`
Expected: FAIL (function does not exist)

- [ ] **Step 3: Write minimal implementation (GREEN)**

Add to `internal/autoscaler/autoscaler.go`:
...
```bash
git add internal/autoscaler/autoscaler.go internal/autoscaler/detect_test.go
...
**Files:** `internal/provider/provider.go`, `internal/provider/ack/ack.go`
...
**Test file:** `internal/provider/ack/ack_test.go`
...
```go
// internal/provider/ack/ack_test.go
package ack_test
...
Run: `go test ./internal/provider/... -v -run TestACKProvider`
...
type Provider interface {
	...
	// Returns true if at least one node has the provider's nodepool label.
...
// providers is the registry of all known providers.
...
Run: `go tool golangci-lint run ./internal/provider/...`
Expected: zero issues

- [ ] **Step 6: Commit**

```bash
git add internal/provider/provider.go internal/provider/ack/ack.go internal/provider/ack/ack_test.go
...
1. Add provider auto-detection and 401 handler to `internal/app/app.go`'s `Run()` function
2. Add controller wiring to `Run()` function

Update the `Run()` function in `internal/app/app.go` to:
- Wrap rest.Config with 401 exit handler (prevents silent dead controller)
- Auto-detect provider from cluster node labels (no `--provider` flag needed)
...
func Run(cfg *rest.Config, bindAddr string, opts Options) error {
	...
	if err != nil {
		return fmt.Errorf("provider detection failed: %w", err)
	}
...
// exitOnUnauthorized wraps an http.RoundTripper to exit the process on 401.
// When a projected ServiceAccount token becomes invalid (e.g. SA deleted and
// recreated during Helm upgrade), client-go retries silently with the stale
// token. The controller stays Running/Ready but does nothing. Exiting forces
// kubelet to restart the pod with a fresh token.
...
func (t *unauthorizedRoundTripper) RoundTrip(req *http.Request) (*http.Response, error) {
	...
	if err == nil && resp.StatusCode == http.StatusUnauthorized {
		slog.Error("API server returned 401 Unauthorized, exiting to trigger pod restart")
		os.Exit(1)
	}
...
Note: `ack` package is not imported here. `provider.DetectProvider()` handles the registry internally.
