...
**Architecture:** Pure-function scheduling checks in `internal/diagnosis/`, nodepool grouping in `internal/nodepool/`, GOATScaler event reading in `internal/autoscaler/`, Provider interface in `internal/provider/` as the plug-and-play extensibility point for cloud providers, PodReconciler in `internal/controller/` wiring everything together. All scheduling checks are tested as pure functions (no K8s client). Controller integration tested with envtest.
...
internal/
  ...
  diagnosis/
    ...
    taint.go          # CheckTaints(pod, nodeInfo) *Rejection
    selector.go       # CheckNodeSelector(pod, nodeInfo) *Rejection
    affinity.go       # CheckNodeAffinity(pod, nodeInfo) *Rejection
    resource.go       # CheckResources(pod, nodeInfo) *Rejection
    diagnose.go       # DiagnoseNodepool(pod, nodeInfo) NodepoolDiagnosis
    diagnose_all.go   # DiagnoseAll(pod, []NodepoolInfo) []NodepoolDiagnosis
  nodepool/
    collector.go      # Collector: groups nodes by label, resolves names
  autoscaler/
    autoscaler.go     # StatusReader interface + detector
    goatscaler.go     # GOATScaler implementation: reads events
  controller/
    reconciler.go     # PodReconciler: watches pods, runs diagnosis, emits events
    setup.go          # SetupWithManager wiring
```
...
## Task 13: Wire reconciler into main.go

**File:** `cmd/main.go`

**What to change:**

1. Import `internal/autoscaler`, `internal/controller`, `internal/provider`, and `internal/provider/ack`
2. After `createManager()`, resolve the provider from the `--provider` flag (default "ack")
3. Call `provider.NewStatusReader(ctx, mgr.GetClient())` to get the autoscaler status reader
4. Create `PodReconciler` with the provider
5. Call `controller.SetupWithManager(mgr, reconciler)`
6. The `--name-label` flag can remain as an override

Replace the `--nodepool-label` and `--autoscaler-configmap` flags with a single `--provider` flag (default "ack"). The `--name-label` flag can remain as an override.
...
func main() {
	...
	if err != nil {
		slog.Error("unable to detect autoscaler", "error", err)
		os.Exit(1)
	}
	...
	if err := controller.SetupWithManager(mgr, reconciler); err != nil {
		slog.Error("unable to setup controller", "error", err)
		os.Exit(1)
	}
...
The `--provider` flag and `--name-label` override are already included in `parseFlags` above.

### Steps

- [ ] **Step 1: Write failing test (RED)**

Verify that `cmd/main_test.go` still compiles. If no test exists, confirm the build compiles:
...
Apply the changes described above to `cmd/main.go`. The full updated file:
...
func setupReconciler(mgr ctrl.Manager, opts managerOptions, prov provider.Provider, reader autoscaler.StatusReader) error {
	reconciler := &controller.PodReconciler{
		Client:          mgr.GetClient(),
		Recorder:        mgr.GetEventRecorderFor("fitcheck"),
		Provider:        prov,
...
func parseFlags() managerOptions {
	...
	flag.StringVar(&opts.providerName, "provider", "ack", "cloud provider (ack, gke, eks)")
	flag.StringVar(&opts.nameLabel, "name-label", "name", "override label key for nodepool human-readable name")
...
Run: `go tool golangci-lint run ./cmd/...`
Expected: zero issues
...
```bash
git add cmd/main.go
git commit -m "feat: wire reconciler into main"
```
